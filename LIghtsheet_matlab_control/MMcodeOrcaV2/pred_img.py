...
from model.dataloader import Defocus_dataset_3 ,Normalizer, Augmenter
from model.model import Unet, UNet, NestedUNet,Unet_3plus,Unet3
...
def img_to_batches(img):
    ...
    image_batches = np.zeros((int(num),3,image_size,image_size))
    height_num = int(height/image_size)
    width_num = int(width/image_size)
    
    for i in range(height_num):
        for j in range(width_num):
            image_batches[i*width_num+j,:,:,:] = img[0,:,i*124:i*124+124,j*124:j*124+124]
...
def pred_whole(img):
    ...
    model_name = 'model_Resnet18_3_8nm_5000.pt'

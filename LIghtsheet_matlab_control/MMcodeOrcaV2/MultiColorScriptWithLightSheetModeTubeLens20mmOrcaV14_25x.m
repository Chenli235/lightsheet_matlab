...
portFWDet = 'COM5';
portFWExt = 'COM7';
portBK_PS405nm = 'COM18';
portBK_PS473nm = 'COM15';
portBK_PS556nm = 'COM14';
portObis640nm = 'COM20';
portObis730nm = 'COM23';
portObis514nm = 'COM21';
portArduino = 'COM6';
...
if (avilableLasers(1))
    ...
    laser405nm.filterEmission = 1;
    laser405nm.filterAF = 1;
    laser405nm.filterExcitation = 1;
    laser405nm.posExtLens = [0.1596; -2.2501];
    laser405nm.posDetLens = [0.9571; 1.1065];
    laser405nm.symmetry = [93; 93];
    laser405nm.Delay = [11.86 23.4 39.52 59.1 84.5 118.01; 11.86 23.7 39.52 59.1 84.5 118.01];
    laser405nm.OptimalDelay = [23.7; 23.7];
    laser405nm.FrameRates = [50 25 15 10 7 5];
    laser405nm.serialPort = s405nm;
    laser405nm.ControlType = 'PS';
    laser405nm.minVoltage = [-650; -650];
    laser405nm.maxVoltage = [650; 650];
...
if (avilableLasers(2))
    ...
    laser473nm.posExtLens = [-0.0707; -0.2080];
    laser473nm.posDetLens = [-0.0582; -0.0682];
    laser473nm.symmetry = [79.3; 79.8];
    laser473nm.Delay = [12.16 25.6 40.62 75.1 87.1 121.8; 12.16 25.7 40.62 75.1 77.1 121.8];
    laser473nm.OptimalDelay = [74.9; 72.1];
    laser473nm.FrameRates = [50 25 15 10 7 5];
    laser473nm.serialPort = s473nm;
    laser473nm.ControlType = 'PS';
    laser473nm.minVoltage = [-650; -650];
    laser473nm.maxVoltage = [650; 650];
...
if (avilableLasers(3))
    ...
    laser514nm.posExtLens = [-0.1307; -0.2879];
    laser514nm.posDetLens = [-0.0462; -0.0313];
    laser514nm.symmetry = [79.5; 79.4]; %[92.4 92.4 91.4 91.3 91.5 91.4]
    laser514nm.Delay = [12.16 25.1 40.62 60.9 87.1 121.8; 12.16 25.1 40.62 60.9 87.1 121.8]; %ms
    laser514nm.OptimalDelay = [73.5; 70.6]; %ms
...
if (avilableLasers(4))
    ...
    laser556nm.posExtLens = [-0.0399; -0.23];
    laser556nm.posDetLens = [-0.013; -0.0229];
    laser556nm.symmetry = [79.3; 79.8];%[92.5; 91.1]; %For 25 fps
    laser556nm.Delay = [12.16 25.6 40.62 75.1 87.1 121.8; 12.16 25.7 40.62 75.1 77.1 121.8]; %ms
    laser556nm.OptimalDelay = [75.0; 72.2]; %ms
    laser556nm.FrameRates = [50 25 15 10 7 5];
    laser556nm.serialPort = s556nm;
    laser556nm.ControlType = 'PS';
    laser556nm.minVoltage = [-650; -650];
    laser556nm.maxVoltage = [650; 650];
...
if (avilableLasers(5))
    ...
    laser589nm.posExtLens = [0.1996; -2.3601];
    laser589nm.posDetLens = [0.9571; 0.9571];
    laser589nm.symmetry = [92.5; 91.1];
    laser589nm.Delay = [11.86 24.8 39.52 73.9 84.6 118.01; 11.86 24.8 39.52 73.9 84.6 118.01];
...
if (avilableLasers(6))
    ...
    laser640nm.posExtLens = [-0.0807; -0.3710];
    laser640nm.posDetLens = [-0.0033; -0.0063];
    laser640nm.symmetry = [79.1; 79.3];%[91.4; 91.2]; %For 25 fps
    laser640nm.Delay = [11.86 25.2 39.52 74.2 84.6 118.01; 11.86 25.4 39.52 74.2 84.6 118.01];
    laser640nm.OptimalDelay = [74.8; 73.3];
    laser640nm.FrameRates = [50 25 15 10 7 5];
    laser640nm.serialPort = s640nm;
    laser640nm.ControlType = 'OBIS';
    laser640nm.minVoltage = [-650; -650];
    laser640nm.maxVoltage = [650; 650];   
...
if (avilableLasers(7))
    ...
    laser730nm.posExtLens = [-0.1697; -0.5690];
    laser730nm.posDetLens = [0.01037; -0.0046];
    laser730nm.symmetry = [82.9; 82.3];%[92.4; 93.3];%For 25 fps
    laser730nm.Delay = [11.86 25.5 39.52 75.3 84.6 118.01; 11.86 25.7 39.52 74.8 84.6 118.01];
    laser730nm.OptimalDelay = [75.3; 74.8];
...
count = 1;
%% Live mode press e = 'exit' to abort live mode
%Set the power
%wavelength = 640;
controlParameters.MMC.clearCircularBuffer;
controlParameters.MMC.getRemainingImageCount;
...
if (lightSheetMode)
    ...
    for calibrationPath = 1:2
        ...
        for ii = (numOfActiveLasers):-1:1
            ...
            tolerance = 0.4; %The differenc between the dif
            toleranceE = 0.2;
...
if (manualFocus)
    ...
    for ii = 1:size(xyzScanArrayFocus,2)
        ...
        dx = xyzScanArrayFocus(1,ii) - xPos; dy = xyzScanArrayFocus(2,ii) - yPos; dz = xyzScanArrayFocus(3,ii) - zPos;
